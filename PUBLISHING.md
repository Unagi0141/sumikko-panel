...
配布ページの見た目を変えたら、こちらも撮り直してください。貼られたときに出る
横長の画像（`docs/og.png`）が古いままになります。

```bash
npm run og
```

`tools/og-template.html` を Electron で開いて写真に撮っているだけです。画像編集ソフトは要りません。
文言や色を変えたいときは、そのテンプレートをいじってから撮り直してください。
...
> このとき最初に、**配布ページの版表記が `package.json` と合っているかを検査します**
> （`tools/check-version.js`）。食い違っていればビルドは始まりません。
> 出てきた行を直してから、もう一度実行してください。
...
`docs/index.html` の `v0.9.0 ベータ · 約 106 MB` と書いてある箇所（ダウンロードボタンの脇。
**上と下の 2 か所あります**）を、上げた版に直します。

> **これは版上げと同じ回に必ずやってください。**
> ダウンロードボタンの飛び先は `releases/latest` です。リリースした瞬間、ボタンは
> 新しい版を渡します。ページの表記だけ古いままだと、**「v0.9.0 と書いてあるボタンが
> v0.9.1 を渡す」状態**になります。ページが、配っている物について事実と違うことを言う——
> 2026-09-08 に実際にこれが起きました。**別の回に回すと、その間ずっとずれています。**
...
食い違っていれば、**どの行が何と書いてあるかを出して止まります。**
この検査は次の `npm run dist` の前にも自動で走るので、**直し忘れたままではインストーラを作れません。**
...
いつ・何を・どこへ出したかを残します。**追記のみで、過去の行は書き換えません。**
（様式は `KSD/本社/platform/公開判定.md` 第3条）
